...
import { ArrowLeft, Mail, Globe, Github, Linkedin, BookOpen, GraduationCap } from 'lucide-react'
...
export default async function MemberProfilePage({ params }: PageProps) {
  ...
  const name = locale === 'en' ? member.name_en || member.name_pt : member.name_pt
  const bio = locale === 'en' ? member.bio_en || member.bio_pt : member.bio_pt
  const currentRole = locale === 'en' ? member.current_role_en || member.current_role_pt : member.current_role_pt
  ...
  const roleLabels: Record<string, string> = {
    ...
    undergrad: locale === 'en' ? 'Undergraduate Researcher' : 'Iniciação Científica',
  ...
  const projects = memberProjects?.map((mp) => mp.project).filter(Boolean) || []
  const publications = memberPublications?.map((mp) => mp.publication).filter(Boolean) || []
          ...
          {projects.length > 0 && (
            <div>
              ...
              <div className="grid gap-4">
                {projects.map((project: { id: string; slug: string; title_pt: string; title_en: string; status: string }) => (
                  <Card key={project.id}>
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        >
                          {locale === 'en' ? project.title_en : project.title_pt}
                        </Link>
