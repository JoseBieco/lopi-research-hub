...
export default async function NewsArticlePage({ params }: PageProps) {
  ...
  const title = locale === 'en' ? news.title_en || news.title_pt : news.title_pt
  const content = locale === 'en' ? news.content_en || news.content_pt : news.content_pt
  const excerpt = locale === 'en' ? news.excerpt_en || news.excerpt_pt : news.excerpt_pt
  ...
  const { data: relatedNews } = await supabase
    .from('news')
    .select('id, slug, title_pt, title_en, published_at, type')
    .eq('is_published', true)
    .neq('id', news.id)
    .eq('type', news.type)
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <header className="space-y-4 mb-8">
            <div className="flex items-center gap-3 flex-wrap">
              <Badge className={typeColors[news.type] || typeColors.general}>
          ...
          <div className="prose prose-slate prose-lg max-w-none">
            {content ? (
              ...
              <p className="text-muted-foreground">
                {locale === 'en' ? 'No content available.' : 'Conteúdo não disponível.'}
              </p>
            )}
          </div>
              ...
              {relatedNews.map((item) => (
                ...
                >
                  ...
                  <h3 className="font-medium hover:text-primary transition-colors">
                    {locale === 'en' ? item.title_en || item.title_pt : item.title_pt}
                  </h3>
                </Link>
              ))}
            </div>
